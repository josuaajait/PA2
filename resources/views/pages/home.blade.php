...
@section('title', 'Caldera Resto & Pool')
    ...
    <div class="row align-items-center">
        ...
        <div class="col-md-7 ps-md-5">
            ...
            <div class="mt-4 d-flex gap-3">
                <a href="{{ route('reservation.table') }}" class="btn btn-home-primary px-4 py-2">Book Table</a>
                <a href="{{ route('reservation.ticket') }}" class="btn btn-home-outline px-4 py-2">Buy Ticket</a>
            </div>
        </div>
    </div>
</div>
        ...
        @foreach($stats as $stat)
            <div class="col-lg-3 col-md-6">
                <div class="card border-0 shadow-sm h-100 text-center p-4 rounded-4 stat-card">
                    <i class="fas {{ $stat['icon'] }} fa-2x mb-3" style="color: #c1a067;"></i>
                    <div class="stats-number mb-1" id="{{ $stat['id'] }}" style="color: #1c3451;">0</div>
                    <p class="fw-bold small text-uppercase mb-0" style="color: #6b7280; letter-spacing: 0.5px;">{{ $stat['label'] }}</p>
                </div>
            </div>
        @endforeach
    </div>
            ...
            <div class="col-md-4">
                <div class="card h-100 border-0 shadow-sm p-4 text-center rounded-4 feature-card">
                    <div class="feature-icon-wrap mx-auto mb-3">
                        <i class="fas {{ $feature['icon'] }} fa-2x" style="color: #c1a067;"></i>
                    </div>
                    <h5 class="fw-bold" style="color: #1c3451;">{{ $feature['title'] }}</h5>
            ...
            @foreach([
                ...
                <div class="carousel-item {{ $index === 0 ? 'active' : '' }}">
                    <div class="text-center px-lg-5">
                        ...
                        <p class="fst-italic text-secondary fs-5">"{{ $testimonial['quote'] }}"</p>
                        <h6 class="fw-bold" style="color: #1c3451;">{{ $testimonial['name'] }}</h6>
                    </div>
                </div>
            @endforeach
        </div>
...
<div class="container mb-5">
    <div class="p-5 text-center text-white rounded-5" style="background: linear-gradient(135deg, #1c3451 0%, #01516e 100%);">
        ...
        <a href="{{ route('reservation.table') }}" class="btn px-5 py-3 fw-bold rounded-pill"
